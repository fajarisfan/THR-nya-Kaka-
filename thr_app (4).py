...
WA_TRAP = "https://wa.me/" + WA_NUMBER + "?text=Haii%20gua%20mau%20ambil%20hadiah%20rod%20Fish%20It%20nih%20%F0%9F%8E%A3"
...
if is_zizah:
    ...
    # Copy DANA + tombol ranjau muncul setelah copy
    components.html(f"""
    <style>
        ...
        .wrap {{
            ...
            border: 2px solid #a855f7; border-radius: 16px; margin-bottom: 1rem;
        }}
        .label {{ color: #c084fc; font-size: 0.85rem; font-family: 'Plus Jakarta Sans', sans-serif; margin-bottom: 0.4rem; }}
        .num {{
            font-size: 2rem; font-weight: 700; color: #e879f9; letter-spacing: 3px;
            background: #1a0025; padding: 0.6rem 1.2rem; border-radius: 10px;
            display: inline-block; border: 1px dashed #a855f7; margin: 0.4rem 0; font-family: monospace;
        }}
        .btn {{
            margin-top: 0.8rem; background: #a855f7; color: #fff; font-weight: 700;
            font-size: 1rem; padding: 0.7rem 2rem; border-radius: 50px; border: none;
        ...
        .gift-box {{
            ...
            border: 2px solid #84cc16; border-radius: 16px;
            padding: 1.5rem; text-align: center; margin-top: 1rem;
            display: none;
            animation: fadeIn 0.5s ease;
        }}
        .gift-box.show {{ display: block; }}
        @keyframes fadeIn {{
            from {{ opacity: 0; transform: translateY(10px); }}
            to {{ opacity: 1; transform: translateY(0); }}
        }}
        .gift-title {{ color: #bef264; font-size: 1rem; font-weight: 700; margin-bottom: 0.3rem; font-family: 'Plus Jakarta Sans', sans-serif; }}
        .gift-sub {{ color: #666; font-size: 0.8rem; font-family: 'Plus Jakarta Sans', sans-serif; margin-bottom: 0.8rem; }}
        .gift-btn {{
    ...
    <div class="gift-box" id="giftBox">
        <div class="gift-title">🎁 Ada hadiah spesial buat kamu!</div>
        <div class="gift-sub">Makasih ya udah mau transfer 🥹 ini hadiahnya ✨</div>
    ...
    <script>
        function copyNum() {{
            navigator.clipboard.writeText('{DANA_NUMBER}').then(function() {{
                ...
                // Munculkan tombol hadiah setelah 1 detik
                setTimeout(function() {{
